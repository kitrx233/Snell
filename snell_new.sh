...
check_new_ver(){
	...
	new_ver="v4.1.1"
	echo -e "${Info} 检测到 Snell Server 最新版本为 [ ${new_ver} ]"
}
...
check_ver_comparison(){
	...
	if [[ "${now_ver}" != "v4.1.1" ]]; then
		echo -e "${Info} 发现 Snell Server 已有新版本 [ v4.1.1 ]"
...
backup_Download() {
	echo -e "${Info} 尝试备用下载方式..."
	# 使用 GitHub 或其他镜像下载（如果可用）
	# 这里可以添加备用下载源，目前保持与主下载相同
	wget --no-check-certificate -N "https://dl.nssurge.com/snell/snell-server-v4.1.1-linux-${arch}.zip"
	if [[ ! -e "snell-server-v4.1.1-linux-${arch}.zip" ]]; then
		echo -e "${Error} 备用下载也失败！请检查网络连接或稍后重试。"
		return 1 && exit 1
	else
		unzip -o "snell-server-v4.1.1-linux-${arch}.zip"
		if [[ ! -e "snell-server" ]]; then
			...
			mv -f snell-server "${FILE}"
			echo "v4.1.1" > ${Now_ver_File}
			echo -e "${Info} Snell Server 备用下载安装完毕！"
			return 0
		fi
	fi
}
...
==============================
——————————————————————————————
 ${Green_font_prefix} 1.${Font_color_suffix} 安装 Snell Server${Yellow_font_prefix}[v4.1.1]${Font_color_suffix}
